...
void ParseDpdkArchitecture::modelError(const char* format, const IR::Node* node) {
    ::error(format, node);
    ::error("Are you using an up-to-date dpdk.p4?");
}

bool ParseDpdkArchitecture::preorder(const IR::PackageBlock* main) {
    for (int i = 0; i < 256; i++) {
        std::string pipeName = "PIPELINE" + std::to_string(i);
        auto pipe = main->findParameterValue(pipeName);
        if (pipe == nullptr) break;
        if (!pipe->is<IR::PackageBlock>()) {
            modelError("%1%: main package does not match the expected dpdk model", main);
            return false;
        }
        auto pipeBlock = pipe->to<IR::PackageBlock>();

        auto prsr = pipeBlock->findParameterValue("prsr");
        if (prsr == nullptr || !prsr->is<IR::ParserBlock>()) {
            modelError("%1%: pipe package does not match the expected dpdk model", pipeBlock);
            return false;
        }
        structure->parsers.emplace(pipeName, prsr->to<IR::ParserBlock>()->container);

        auto ingress = pipeBlock->findParameterValue("ma");
        if (ingress == nullptr || !ingress->is<IR::ControlBlock>()) {
            modelError("%1%: pipe package does not match the expected dpdk model", pipeBlock);
            return false;
        }
        structure->pipes.emplace(pipeName, ingress->to<IR::ControlBlock>()->container);
        auto ingress_name = ingress->to<IR::ControlBlock>()->container->name;
        structure->pipeline_controls.emplace(ingress_name);

        auto deparser = pipeBlock->findParameterValue("dprsr");
        if (deparser == nullptr || !deparser->is<IR::ControlBlock>()) {
            modelError("%1%: pipe package does not match the expected dpdk model", pipeBlock);
            return false;
        }
        structure->deparsers.emplace(pipeName, deparser->to<IR::ControlBlock>()->container);
...
void
DpdkSwitchBackend::modelError(const char* format, const IR::Node* node) const {
    ::error(format, node);
    ::error("Are you using an up-to-date v1model.p4?");
}

cstring
DpdkSwitchBackend::createCalculation(cstring algo, const IR::Expression* fields,
...
void
DpdkSwitchBackend::convertChecksum(const IR::BlockStatement *block, Util::JsonArray* checksums,
...
void DpdkSwitchBackend::createActions(ConversionContext* ctxt, DpdkProgramStructure* structure) {
...
void
DpdkSwitchBackend::convert(const IR::ToplevelBlock* tlb) {
    structure = new DpdkProgramStructure();

    auto parseV1Arch = new ParseDpdkArchitecture(structure);
    auto main = tlb->getMain();
    if (!main) return;  // no main

    if (main->type->name != "Pipelines")
        ::warning("%1%: the main package should be called Pipelines"
    ...
    auto pipe0 = main->findParameterValue("PIPELINE0")->to<IR::PackageBlock>();
    CHECK_NULL(pipe0);

    // Find the user metadata declaration
    auto parser = pipe0->findParameterValue("prsr");
    ...
    conv = new DpdkSwitchExpressionConverter(refMap, typeMap, structure, scalarsName);
    ...
    for (const auto& parser : structure->parsers) {
        cstring name = parser.first + ".prsr";
        auto pconv = new ParserConverter(ctxt, name);
        parser.second->apply(*pconv);
    }

    createActions(ctxt, structure);

    for (const auto& pipe : structure->pipes) {
        cstring name = pipe.first + ".ma";
        auto cconv = new ControlConverter(ctxt, name, options.emitExterns);
        pipe.second->apply(*cconv);
    }

    for (const auto& deparser : structure->deparsers) {
        cstring name = deparser.first + ".dprsr";
        auto dconv = new DeparserConverter(ctxt, name);
        deparser.second->apply(*dconv);
    }
